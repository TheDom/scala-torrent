...
class TrackerResponseSpec extends UnitSpec {
  ...
  "TrackerResponse" should "parse a correct response" in {
    ...
    TrackerResponse(str) should be (TrackerResponseWithSuccess(1625, None, None, 1, 0, List(Peer(Some("-<30000-bitlove.org/"), "10.0.0.2", 6881)), None))
  }

  it should "parse a response with a failure" in {
    val str = "d14:failure reason17:incorrect requeste"
    TrackerResponse(str) should be (TrackerResponseWithFailure("incorrect request"))
  }
}
